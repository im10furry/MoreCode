...
impl Explorer {
    ...
    async fn scan_project(&self, root: &Path) -> Result<ProjectContext, AgentError> {
        ...
        task::spawn_blocking(move || Self::scan_sync(root, config))
            .await
            .map_err(|e| AgentError::ExecutionFailed { agent_type: AgentType::Explorer, message: e.to_string() })?
    }
    ...
    fn scan_sync(root: PathBuf, config: AgentConfig) -> Result<ProjectContext, AgentError> {
        ...
        fs::create_dir_all(&memory_dir).map_err(|e| AgentError::io(memory_dir.display().to_string(), e))?;
        let cache_path = memory_dir.join("explorer-cache.json");
        let cache = if cache_path.exists() {
            let payload = fs::read_to_string(&cache_path).map_err(|e| AgentError::io(cache_path.display().to_string(), e))?;
            Some(serde_json::from_str::<ScanCache>(&payload).map_err(AgentError::serialization)?)
        } else {
            None
        };

        let files = Self::collect_files(&root, config.explorer.max_files, config.explorer.max_file_size_bytes)?;
        let mut records = BTreeMap::new();
        let mut changed = Vec::new();
        for file in files {
            let rel = Self::norm(file.strip_prefix(&root).map_err(|e| AgentError::io(file.display().to_string(), e))?);
        ...
        let cache_fresh = cache
            .as_ref()
            .map(|cache| (Utc::now() - cache.saved_at).num_seconds() <= config.explorer.cache_ttl_secs as i64)
            .unwrap_or(false);
        if changed.is_empty() && cache_fresh {
            return cache.map(|cache| cache.project_context).ok_or_else(|| AgentError::ContextBuildFailed { message: "cache missing".to_string() });
        }

        let (facts, framework) = Self::cargo_facts(&root)?;
        let modules = Self::modules(&records, &facts);
        let graph = Self::graph(&modules);
        let primary_language = if records.values().any(|record| record.language == "Rust") { "Rust".to_string() } else { "Unknown".to_string() };
        let total_lines = records.values().map(|record| record.line_count).sum();
        let scan_mode = if !changed.is_empty() && (changed.len() as f32 / records.len().max(1) as f32) <= config.explorer.incremental_change_threshold {
            ScanMode::Incremental { changed_files: changed.clone() }
        } else {
            ScanMode::Full
        };
        ...
        let context = ProjectContext {
            project_info: ProjectInfo {
                name: root.file_name().map(|v| v.to_string_lossy().into_owned()).unwrap_or_else(|| "workspace".to_string()),
            ...
            structure: ProjectStructure {
                directory_tree: Self::tree(records.keys().cloned().collect(), config.explorer.max_tree_depth),
                total_files: records.len(),
                total_lines,
                entry_files: records.values().filter(|record| record.is_entry).map(|record| record.relative_path.clone()).collect(),
                config_files: records.values().filter(|record| record.is_config).map(|record| record.relative_path.clone()).collect(),
                modules: modules.clone(),
            },
            tech_stack: TechStack {
                language_version: facts.get("rust-toolchain").cloned().unwrap_or_else(|| primary_language.clone()),
            ...
            architecture: ArchitecturePattern {
                ...
                description: format!("{primary_language} workspace with {} modules", modules.len()),
            ...
            conventions: CodeConventions {
                ...
                error_handling: ErrorHandlingPattern {
                    ...
                    unwrap_policy: if records.values().any(|record| record.risk_markers.iter().any(|marker| marker == "unwrap")) { "Prefer Result propagation over unwrap()".to_string() } else { "Prefer Result propagation".to_string() },
            ...
            scan_metadata: ScanMetadata {
                ...
                memory_version: 1,
                scanner_version: "mc-agent.explorer.v1".to_string(),
        ...
        let cache = ScanCache { version: 1, saved_at: Utc::now(), root_path: context.root_path.clone(), records, project_context: context.clone() };
        let payload = serde_json::to_string_pretty(&cache).map_err(AgentError::serialization)?;
        fs::write(&cache_path, payload).map_err(|e| AgentError::io(cache_path.display().to_string(), e))?;
        Ok(context)
    }

    fn collect_files(root: &Path, max_files: usize, max_size: u64) -> Result<Vec<PathBuf>, AgentError> {
        let mut builder = WalkBuilder::new(root);
        builder.hidden(false).parents(true).git_ignore(true).git_exclude(true).git_global(true).standard_filters(true);
        let mut files = Vec::new();
        for entry in builder.build() {
            let Ok(entry) = entry else { continue };
            if !entry.file_type().map(|t| t.is_file()).unwrap_or(false) { continue; }
            let path = entry.into_path();
            let rel = Self::norm(path.strip_prefix(root).map_err(|e| AgentError::io(path.display().to_string(), e))?);
            if rel.starts_with(".git/") || rel.starts_with(".assistant-memory/") || rel.starts_with("target/") || rel.starts_with("node_modules/") { continue; }
            let keep = Self::is_manifest(&rel) || matches!(Path::new(&rel).extension().and_then(|v| v.to_str()).unwrap_or_default(), "rs" | "toml" | "md" | "json");
            if !keep { continue; }
            let meta = fs::metadata(&path).map_err(|e| AgentError::io(path.display().to_string(), e))?;
            if meta.len() > max_size && !Self::is_manifest(&rel) { continue; }
            files.push(path);
    ...
    fn analyze_file(root: &Path, file: &Path, hash: u64) -> Result<CachedFileRecord, AgentError> {
        let rel = Self::norm(file.strip_prefix(root).map_err(|e| AgentError::io(file.display().to_string(), e))?);
        let content = String::from_utf8_lossy(&fs::read(file).map_err(|e| AgentError::io(file.display().to_string(), e))?).into_owned();
        let use_re = Regex::new(r"use\s+([a-zA-Z0-9_]+)").expect("regex");
        let export_re = Regex::new(r"pub\s+(?:async\s+)?(?:fn|struct|enum|trait)\s+([A-Za-z0-9_]+)").expect("regex");
        Ok(CachedFileRecord {
            relative_path: rel.clone(),
            hash,
            size_bytes: content.len() as u64,
            line_count: content.lines().count(),
            language: if rel.ends_with(".rs") { "Rust".to_string() } else { "Other".to_string() },
            ...
            dependencies: use_re.captures_iter(&content).filter_map(|c| c.get(1)).map(|m| m.as_str().to_string()).collect::<BTreeSet<_>>().into_iter().collect(),
            exports: export_re.captures_iter(&content).filter_map(|c| c.get(1)).map(|m| m.as_str().to_string()).collect::<BTreeSet<_>>().into_iter().collect(),
            risk_markers: ["unsafe", "todo", "fixme", "unwrap("].into_iter().filter(|n| content.to_lowercase().contains(*n)).map(|n| if n == "unwrap(" { "unwrap" } else { n }).map(str::to_string).collect(),
        })
    }

    fn cargo_facts(root: &Path) -> Result<(HashMap<String, String>, Option<String>), AgentError> {
        let root_manifest = root.join("Cargo.toml");
        let payload = fs::read_to_string(&root_manifest).map_err(|e| AgentError::io(root_manifest.display().to_string(), e))?;
        let value: toml::Value = toml::from_str(&payload).map_err(AgentError::serialization)?;
        let members = value.get("workspace").and_then(|v| v.get("members")).and_then(toml::Value::as_array).cloned().unwrap_or_default();
        let mut facts = HashMap::new();
        let mut framework = None;
        if root.join("rust-toolchain.toml").exists() {
            let payload = fs::read_to_string(root.join("rust-toolchain.toml")).map_err(|e| AgentError::io("rust-toolchain.toml", e))?;
            let value: toml::Value = toml::from_str(&payload).map_err(AgentError::serialization)?;
            if let Some(channel) = value.get("toolchain").and_then(|t| t.get("channel")).and_then(toml::Value::as_str) {
                facts.insert("rust-toolchain".to_string(), channel.to_string());
            }
        }
        for member in members.iter().filter_map(toml::Value::as_str) {
            let manifest = root.join(member).join("Cargo.toml");
            let payload = fs::read_to_string(&manifest).map_err(|e| AgentError::io(manifest.display().to_string(), e))?;
            let value: toml::Value = toml::from_str(&payload).map_err(AgentError::serialization)?;
            if let Some(edition) = value.get("package").and_then(|p| p.get("edition")).and_then(toml::Value::as_str) {
                facts.entry("edition".to_string()).or_insert_with(|| edition.to_string());
            }
            if let Some(table) = value.get("dependencies").and_then(toml::Value::as_table) {
                for (name, dep) in table {
                    let version = dep.as_str().map(ToOwned::to_owned).unwrap_or_else(|| "workspace".to_string());
                    facts.insert(name.clone(), version);
                    if framework.is_none() && ["axum", "actix-web", "ratatui", "tauri", "bevy"].contains(&name.as_str()) {
    ...
    fn modules(records: &BTreeMap<String, CachedFileRecord>, facts: &HashMap<String, String>) -> Vec<ModuleInfo> {
        let mut grouped: HashMap<String, Vec<&CachedFileRecord>> = HashMap::new();
        for record in records.values() { grouped.entry(record.module.clone()).or_default().push(record); }
        let mut modules = grouped.into_iter().map(|(path, files)| ModuleInfo {
            name: path.trim_start_matches("crates/").to_string(),
            path,
            description: "Discovered module".to_string(),
            exports: files.iter().flat_map(|f| f.exports.clone()).collect::<BTreeSet<_>>().into_iter().collect(),
            dependencies: files.iter().flat_map(|f| f.dependencies.clone()).filter(|d| facts.contains_key(d)).collect::<BTreeSet<_>>().into_iter().collect(),
    ...
    fn graph(modules: &[ModuleInfo]) -> DependencyGraph {
        let by_name = modules.iter().map(|m| (m.name.clone(), m.path.clone())).collect::<HashMap<_, _>>();
        let edges = modules.iter().flat_map(|m| m.dependencies.iter().filter_map(|d| by_name.get(d).map(|to| DependencyEdge {
            from: m.path.clone(),
            to: to.clone(),
            relation_type: "compile".to_string(),
            weight: Some(1.0),
        }))).collect::<Vec<_>>();
        DependencyGraph { nodes: modules.iter().map(|m| m.path.clone()).collect(), edges, circular_dependencies: Vec::new() }
    }
    ...
    fn tree(paths: Vec<String>, max_depth: usize) -> String {
        ...
        for path in paths {
            ...
            lines.push(format!("{}- {}", "  ".repeat(depth.saturating_sub(1)), segs[..depth].join("/")));
        }
        lines.join("\n")
    }
    ...
    fn is_manifest(rel: &str) -> bool {
        matches!(rel, "Cargo.toml" | "Cargo.lock" | ".gitignore" | "README.md" | "rust-toolchain.toml") || rel.ends_with("/Cargo.toml")
    }
    ...
    async fn enrich(&self, ctx: &AgentContext, project_ctx: &mut ProjectContext) -> Result<u32, AgentError> {
        let prompt = format!("name: {}\nfiles: {}\nmodules: {}\ntree:\n{}", project_ctx.project_info.name, project_ctx.structure.total_files, project_ctx.structure.modules.iter().map(|m| m.name.as_str()).collect::<Vec<_>>().join(", "), project_ctx.structure.directory_tree);
        ...
        project_ctx.architecture.design_decisions.extend(summary.design_decisions);
        project_ctx.conventions.custom_rules.extend(summary.notable_patterns);
        Ok(tokens)
    }
}
...
impl Agent for Explorer {
    ...
    async fn build_context(&self, task: &TaskDescription, project_ctx: Option<ProjectContext>, shared: &SharedResources) -> Result<AgentContext, AgentError> {
        if let Some(project_ctx) = project_ctx {
            return Ok(AgentContext::new(task.clone(), shared, self.config.clone()).with_project_ctx(project_ctx));
        }
        let root = task.project_root_path().unwrap_or_else(|| shared.project_root.clone());
    ...
    async fn execute(&self, ctx: &AgentContext) -> Result<AgentExecutionReport, AgentError> {
        let mut project_ctx = ctx.project_ctx.as_deref().cloned().ok_or_else(|| AgentError::MissingContextData { data_type: "ProjectContext".to_string() })?;
        ...
        Ok(AgentExecutionReport::success(AgentType::Explorer, &ctx.execution_id, format!("Explorer scanned {} files in {}", project_ctx.structure.total_files, project_ctx.scan_metadata.scan_mode), result, ctx.elapsed_ms(), tokens))
    }
}
...
mod tests {
    ...
    async fn explorer_marks_incremental_scan_and_uses_json_schema() {
        ...
        let first = explorer.build_context(&task, None, &shared).await.expect("first ctx");
        explorer.execute(&first).await.expect("first run");

        std::fs::write(
            project.path().join("crates/core/src/lib.rs"),
            "pub fn compute() -> usize { 7 }\n",
        )
        .expect("rewrite");

        let second = explorer.build_context(&task, None, &shared).await.expect("second ctx");
        ...
        assert!(project_ctx
            ...
            .changed_files
            .iter()
            .any(|path| path == "crates/core/src/lib.rs"));
